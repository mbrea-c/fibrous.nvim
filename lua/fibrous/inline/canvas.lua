...
-- Reset every cell of `rect` to a blank, hl-less cell — the state a fresh
-- canvas starts in. The incremental painter (render.update) blanks a changed
-- subtree's old and new areas before restoring ancestor backgrounds and
-- repainting, so stale chars and hls can't shine through.
---@param rect { x: integer, y: integer, w: integer, h: integer }
function Canvas:blank_rect(rect)
	for y = math.max(rect.y, 0), math.min(rect.y + rect.h, self.h) - 1 do
		local chr, cwr, hlr = self.ch[y + 1], self.cw[y + 1], self.hl[y + 1]
		local x0, x1 = math.max(rect.x, 0), math.min(rect.x + rect.w, self.w) - 1
		-- a wide char straddling the rect edge loses one half: blank the other
		if x0 > 0 and cwr[x0 + 1] == 0 then
			chr[x0], cwr[x0] = " ", 1
		end
		if x1 + 1 < self.w and cwr[x1 + 2] == 0 then
			chr[x1 + 2], cwr[x1 + 2] = " ", 1
		end
		for x = x0, x1 do
			chr[x + 1], cwr[x + 1], hlr[x + 1] = " ", 1, nil
		end
	end
end
...
-- One row as a buffer line (1-based), for patching retained line arrays.
---@param y integer
---@return string
function Canvas:line(y)
	return table.concat(self.ch[y])
end
...
function Canvas:highlights()
	local out = {}
	for y = 1, self.h do
		self:row_spans(y, out)
	end
	return out
end

-- One row's highlight spans (1-based row), appended to `out` (or a new list).
---@param y integer
---@param out? table[]
---@return { row: integer, start_col: integer, end_col: integer, hl: string }[]
function Canvas:row_spans(y, out)
	out = out or {}
