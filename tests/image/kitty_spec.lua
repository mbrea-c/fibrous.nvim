...
describe("kitty.query", function()
  local kitty = require("fibrous.image.kitty")

  it("builds a graphics capability query (a=q) with the given id", function()
    local q = kitty.query(31)
    assert.truthy(q:find("^\27_G"))
    assert.truthy(q:find("a=q", 1, true))
    assert.truthy(q:find("i=31", 1, true))
    assert.truthy(q:find("s=1,v=1", 1, true))
    assert.truthy(q:find("f=24", 1, true))
    assert.truthy(q:find(";AAAA\27\\", 1, true))
  end)

  it("never suppresses the response (no q= key: the reply IS the point)", function()
    assert.falsy(kitty.query(31):find("q=", 1, true))
  end)
end)
